...
#import "MGPanGestureRecognizer.h"
...
@interface MGPanNavigationController ()<UIGestureRecognizerDelegate>
@property (strong, nonatomic) MGPanGestureRecognizer *pan;
...
@implementation MGPanNavigationController
...
- (void)viewDidLoad{
    ...
    _pan = [[MGPanGestureRecognizer alloc] initWithTarget:self action:@selector(pan:)];
    _pan.delegate = self;
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if ([[AppDelegate getAppDelegate].isscroll isEqualToString:@"0"]) {
        succeed = NO;
    }
...
- (BOOL)enablePanBack
{
    ...
    if ([self.visibleViewController isKindOfClass:[BarRootViewController class]]) {
        enable = NO;
    }
    return enable;
}
