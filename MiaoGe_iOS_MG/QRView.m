...
- (void)layoutSubviews {
    ...
    if(!bgimg){
        bgimg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"my_icon_img"]];
        bgimg.center = CGPointMake(ViewSizeW/2, ViewSizeH/2 - self.transparentArea.width / 2 - bgimg.frame.size.height/2 - 50 );
        [self addSubview:bgimg];
    }
    
    if(!titlelbl){
        titlelbl = [[UILabel alloc] init];
        [UtilClass lblatrr:titlelbl font:FontSystem_18 color:Color_Main alignment:NSTextAlignmentLeft text:@"扫一扫"];
        [titlelbl sizeToFit];
        titlelbl.center = CGPointMake(ViewSizeW/2, ViewSizeH/2 + self.transparentArea.width / 2 + 15 + titlelbl.frame.size.height/2 - 50);
...
- (void)initQRLine {
    ...
    qrLine.image = [UIImage imageNamed:@"my_scanline_img"];
...
- (void)show {
    ...
    [UIView animateWithDuration:kQrLineanimateDuration animations:^{
        ...
        CGFloat maxBorder = self.frame.size.height / 2 + self.transparentArea.height / 2 - 4 - 50 ;
...
- (void)addScreenFillRect:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    CGContextSetRGBFillColor(ctx, 0 / 255.0,0 / 255.0,0 / 255.0,0.6);
...
- (void)addCornerLineWithContext:(CGContextRef)ctx rect:(CGRect)rect{
    ...
    CGContextSetRGBStrokeColor(ctx, 73 /255.0, 173/255.0, 182/255.0, 1);//绿色
